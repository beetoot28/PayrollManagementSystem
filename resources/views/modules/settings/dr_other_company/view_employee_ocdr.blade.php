<div class="max-w-7xl mx-auto sm:px-6 lg:px-8" style="display: none;" x-show="active_tab == 3" x-transition:enter.duration.500ms>
    <div class="overflow-hidden shadow-sm sm:rounded-lg h-5/6">
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 relative flex flex-col lg:flex-row lg:space-x-3 mt-3">
            @if ($view_ocdr_record)
                <a href="{{ route('view-employee', $view_ocdr_record->employee->employee_id) }}" class="" title="View Profile">
                    <div class="mb-4">
                        <div class="w-32 h-32 bg-gray-200 rounded-xl flex flex-row border border-gray-600">
                            <div class="flex justify-center items-center mx-auto">
                                <img src="{{ asset('storage/employee/'.$view_ocdr_record->employee->employee_photo) }}" alt="emp_photo">
            ...
            <div class="relative w-full">
                ...
                <div class="flex flex-col lg:flex-row ">
                    <form action="{{ route('view-employee-ocdr', $view_ocdr_record ? $view_ocdr_record->employee->employee_id : '') }}" method="GET" class="flex items-center w-full">
                    ...
                    <form action="{{ route('view-employee-ocdr', $view_ocdr_record ? $view_ocdr_record->employee->employee_id : '') }}" method="GET" class="flex items-center w-full ml-0 lg:ml-3">
                        <div class="flex flex-col lg:flex-row space-x-0 lg:space-x-5 mb-1 w-full">
                            <div class="mb-1 w-full">
        ...
        <div class="max-w-7xl mx-6 mt-2 justify-center flex items-center shadow-md sm:rounded-lg overflow-x-auto">
            <table class="w-full text-sm text-left dark:text-gray-400">
                ...
                <tbody>
                    @if ($view_ocdr_records)
                        @foreach ($view_ocdr_records as $dr_record)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="py-3 text-center border-gray-300 text-gray-900 text-sm p-2">
                                    <button type="button" {{ $dr_record->is_paid ? 'disabled' : '' }} title="Edit" class="text-white bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-lime-300 dark:focus:ring-lime-800 shadow-lg shadow-lime-500/50 dark:shadow-lg dark:shadow-lime-800/80 font-medium rounded-lg text-sm px-5 py-1 text-center mr-2 mb-2"
                                        x-on:click="update_record = true, s_id = '{{ $dr_record->id }}', s_date = '{{ date('F j, Y', strtotime($dr_record->created_at)) }}', s_amount='{{ $dr_record->amount }}', s_note = '{{ $dr_record->note }}', modal_title = 'Edit Other Employee DR'"
                                        >
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
        ...
        <div class="max-w-7xl mx-6 mt-2">
            {{ $view_ocdr_records ? $view_ocdr_records->links() : '' }}
        </div>
