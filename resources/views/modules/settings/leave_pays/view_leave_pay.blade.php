<div class="max-w-7xl mx-auto sm:px-6 lg:px-8" style="display: none;" x-show="active_tab == 5" x-transition:enter.duration.500ms>
    <div class="overflow-hidden shadow-sm sm:rounded-lg h-5/6">
        ...
        <div class="max-w-7xl mx-auto px-6 lg:px-8 relative flex flex-col lg:flex-row lg:space-x-3 mt-3">
            @if ($view_leavepay_record)
                <a href="{{ route('view-employee', $view_leavepay_record->employee->employee_id) }}" class="" title="View Profile">
                    <div class="mb-4">
                        <div class="w-32 h-32 bg-gray-200 rounded-xl flex flex-row border border-gray-600">
                            <div class="flex justify-center items-center mx-auto">
                                <img src="{{ asset('storage/employee/'.$view_leavepay_record->employee->employee_photo) }}" alt="emp_photo">
            ...
            <div class="relative w-full">
                <div class="flex flex-col lg:flex-row space-x-0 lg:space-x-5 mb-3">
                    <div class="mb-1 w-full">
                        ...
                        <input id="" type="text" class=" bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ $view_leavepay_record ? $view_leavepay_record->employee->first_name.' '.$view_leavepay_record->employee->last_name.', '.$view_leavepay_record->employee->middle_name : '' }}" autocomplete="off" disabled>
                    </div>
                    <div class="mb-1 w-full">
                        <label for="time_out_am" class="block text-sm font-medium text-gray-900 dark:text-gray-300">Department</label>
                        <input id="time_out_am" type="text" class=" bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ $view_leavepay_record ? $view_leavepay_record->employee->EmployeeDetails->department : '' }}" autocomplete="off" disabled>
                    </div>
                    <div class="mb-1 w-full">
                        <label for="time_in_pm" class="block text-sm font-medium text-gray-900 dark:text-gray-300">Employee Status</label>
                        <input id="time_in_pm" type="text" class=" bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500"  value="{{ $view_leavepay_record ? $view_leavepay_record->employee->employee_status : '' }}" autocomplete="off" disabled>
                    </div>

                </div>
                <div class="flex flex-col lg:flex-row ">
                    <form action="{{ route('view-employee-leavepay', $view_leavepay_record ? $view_leavepay_record->employee->employee_id : '') }}" method="GET" class="flex items-center w-full">
                        <div class="flex flex-col lg:flex-row space-x-0 lg:space-x-5 mb-1 w-full">
                            <div class="mb-1 w-full">
                                <label for="leavepay_start_date" class="block text-sm font-medium text-gray-900 dark:text-gray-300">Start Date</label>
                                <div class="flex flex-row">
                                    <input datepicker datepicker-autohide id="leavepay_start_date" name="leavepay_start_date" type="text"class=" bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ request()->query('leavepay_start_date') }}" autocomplete="off">
                                </div>
                            </div>
                            <div class="mb-1 w-full">
                                <label for="leavepay_start_date" class="block text-sm font-medium text-gray-900 dark:text-gray-300">End Date</label>
                                <div class="flex flex-row">
                                    <input datepicker datepicker-autohide id="leavepay_end_date" name="leavepay_end_date" type="text" class=" bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ request()->query('leavepay_end_date') }}" autocomplete="off">
                                </div>
                            </div>
                        </div>
                    ...
                    <form action="{{ route('view-employee-leavepay', $view_leavepay_record ? $view_leavepay_record->employee->employee_id : '') }}" method="GET" class="flex items-center w-full ml-0 lg:ml-3">
                        <div class="flex flex-col lg:flex-row space-x-0 lg:space-x-5 mb-1 w-full">
                            <div class="mb-1 w-full">
                                <label for="leavepay_note" class="block text-sm font-medium text-gray-900 dark:text-gray-300">Note</label>
                                <div class="flex flex-row">
                                    <input id="leavepay_note" name="leavepay_note" type="text"class=" bg-gray-50 border border-gray-300 text-gray-900 text-xs rounded-md focus:ring-blue-500 focus:border-blue-500 block w-full px-2 py-2 dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" value="{{ request()->query('leavepay_note') }}" autocomplete="off">
                                </div>
                            </div>
                        </div>
                        <button type="submit" class="px-3 ml-2 text-sm py-1 mt-3 text-white bg-blue-700 rounded-md border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
        ...
        <div class="flex items-center space-x-2 max-w-7xl mx-auto px-6">
            ...
            <a href="{{ route('view-employee-leavepay', $view_leavepay_record ? $view_leavepay_record->employee->employee_id : '') }}" class="inline-flex px-3 py-1.5 text-sm text-white bg-blue-700 rounded-md border border-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800">
        ...
        <div class="max-w-7xl mx-6 mt-2 justify-center flex items-center shadow-md sm:rounded-lg overflow-x-auto">
            <table class="w-full text-sm text-left dark:text-gray-400">
                ...
                <tbody>
                    @if ($view_leavepay_records)
                        @foreach ($view_leavepay_records as $dr_record)
                            <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                                ...
                                <td class="py-3 text-center border-gray-300 text-gray-900 text-sm p-2">
                                    <span>{{ date('F j, Y', strtotime($dr_record->leave_date)) }}</span>
                                </td>
                                <td class="py-3 text-center border-gray-300 text-gray-900 text-sm p-2">
                                    <span>{{ $dr_record->note }}</span>
                                </td>
                                <td class="py-3 text-center border-gray-300 text-gray-900 text-sm p-2">
                                    <button type="button" {{ $dr_record->is_used ? 'disabled' : '' }} title="Edit" class="text-white bg-gradient-to-r from-lime-200 via-lime-400 to-lime-500 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-lime-300 dark:focus:ring-lime-800 shadow-lg shadow-lime-500/50 dark:shadow-lg dark:shadow-lime-800/80 font-medium rounded-lg text-sm px-5 py-1 text-center mr-2 mb-2"
                                        x-on:click="update_record_leavepay = true, s_id = '{{ $dr_record->id }}', s_date = '{{ date('F j, Y', strtotime($dr_record->created_at)) }}', $refs.e_leave_date.value = '{{ date('m/d/Y', strtotime($dr_record->leave_date)) }}', s_note = '{{ $dr_record->note }}', modal_title = 'Edit Employee Leave Pay'"
                                        >
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="w-5 h-5">
                                            <path stroke-linecap="round" stroke-linejoin="round" d="M16.862 4.487l1.687-1.688a1.875 1.875 0 112.652 2.652L10.582 16.07a4.5 4.5 0 01-1.897 1.13L6 18l.8-2.685a4.5 4.5 0 011.13-1.897l8.932-8.931zm0 0L19.5 7.125M18 14v4.75A2.25 2.25 0 0115.75 21H5.25A2.25 2.25 0 013 18.75V8.25A2.25 2.25 0 015.25 6H10" />
                                        </svg>
                                    </button>
                                    <button type="button" {{ $dr_record->is_used ? 'disabled' : '' }} title="Delete" class="text-white bg-gradient-to-r from-red-400 via-red-500 to-red-600 hover:bg-gradient-to-br focus:ring-4 focus:outline-none focus:ring-red-300 dark:focus:ring-red-800 shadow-lg shadow-red-500/50 dark:shadow-lg dark:shadow-red-800/80 font-medium rounded-lg text-sm px-5 py-1 text-center mr-2 mb-2"
        ...
        <div class="max-w-7xl mx-6 mt-2">
            {{ $view_leavepay_records ? $view_leavepay_records->links() : '' }}
        </div>
